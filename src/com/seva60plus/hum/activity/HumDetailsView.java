...
public class HumDetailsView extends BaseActivity implements
		HumDetailsAsyncListener, UpdateHumDetailsAsyncListener {
	...
	private RelativeLayout rl_isGeo, rl_isNotificationEnabled;
	private CheckBox cb_isGeo;
	private CheckBox cb_isNotificationEnabled;
	private StaticConstant mConstant;
	...
	private void initView() {
		...
		rl_isNotificationEnabled = (RelativeLayout) findViewById(R.id.rl_isNotificationEnabled);
		cb_isNotificationEnabled = (CheckBox) findViewById(R.id.cb_isNotification);

		mConstant = PreferenceUtil.fetchStaticData(mContext);
		if (mConstant != null) {

			cb_isNotificationEnabled.setChecked(PreferenceUtil.fetchStaticData(
					mContext).isShowReminder());
		} else
			mConstant = new StaticConstant();
		...
		rl_isNotificationEnabled.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {

				if (cb_isNotificationEnabled.isChecked())
					cb_isNotificationEnabled.setChecked(false);
				else if (!cb_isNotificationEnabled.isChecked())
					cb_isNotificationEnabled.setChecked(true);
			}
		});
	...
	public void getHumDetailsJson(Hum mHum) {
		...
		if (mHum != null) {
			...
			if (mHum.getGender() != null
					&& mHum.getGender().trim().equalsIgnoreCase("M")) {
			...
			if (mHum.getIsGeoEnabled() != null
					&& mHum.getIsGeoEnabled().trim().equalsIgnoreCase("1"))
				cb_isGeo.setChecked(true);
			else
				cb_isGeo.setChecked(false);
	...
	public void onSaveButtonClick(View v) {
		...
		// SAVE Reminder Constant
		mConstant.setShowReminder(cb_isNotificationEnabled.isChecked());
		PreferenceUtil.saveStaticData(HumDetailsView.this, mConstant);
	...
	public void getUpdateStatus(String result, String isGeo) {
		...
		if (result.equalsIgnoreCase(UpdateHumDetails.RESPONSE_SUCCESS)) {
			try {
				Toast.makeText(mContext, "Hum Details Updated.",
						Toast.LENGTH_SHORT).show();
				SharedPreferences.Editor mEditor = Util
						.getSharedEditor(mContext);
	...
	private DatePickerDialog.OnDateSetListener pickerListener = new DatePickerDialog.OnDateSetListener() {
		...
		public void onDateSet(DatePicker view, int selectedYear,
			...
			month = selectedMonth;
			day = selectedDay;
			StringBuilder superStringBuilder = new StringBuilder().append(day)
